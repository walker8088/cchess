...
class TestUCCI():
    def setup_method(self):
        os.chdir(os.path.dirname(__file__))
        self.engine = UcciEngine()
        self.engine.load("../tests/bin/eleeye")

    def teardown_method(self):
        pass

    def test_ucci(self):
        fen, moves, result = load_move_txt('../tests/data/ucci_test1_move.txt')
        game = read_from_xqf('../tests/data/ucci_test1.xqf')
        game.init_board.move_player = ChessPlayer(RED)
...
class TestUCI():
    def setup_method(self):
        ...
        self.engine.load("../tests/bin/pikafish")
