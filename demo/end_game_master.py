...
class Pos:
    """Simple position class with x, y coordinates."""
        ...
        if engine:
            ...
            if output is not None:
                ...
                if action == "bestmove":
                    ...
                    if move is None:
                        print(f"Invalid move: {iccs}")
                        dead = True
                    else:
                        # print(output["move"], move.to_text())
                        # move_iccs already switches the turn, no need for next_turn()
                        move.prepare_for_engine(
                            self.board.move_side(), self.move_history
                        )
                        self.move_history.append(move)
                        move_list_text = ",".join(
                            [m.to_text() for m in self.move_history]
                        )
                        print(move_list_text)
                        has_move = True

                elif action in ["dead", "draw", "resign"]:
                    print(action, self.board.move_side())
                    dead = True
            ...
            if dead:
                ...
                if table.board.has_no_legal_moves():
                    ...
                    if len(table.move_history) > 80:
                        ...
                        engine = table.engine[table.board.move_side()]
