...
class Move(object):
    ...
    def mirror(self):
        self.board.mirror()
        self.p_from.x = 8 - self.p_from.x
        self.p_to.x = 8 - self.p_to.x
    ...
    def to_chinese(self):
        ...
        #王车炮兵规则
        if man_species in [
                PieceT.KING, PieceT.ROOK, PieceT.CANNON, PieceT.PAWN
        ]:
            if diff == 0:
    ...
    def __get_chinese_name(self, p_from):
        ...
        pos_name5 = ((u'后', u'四', u'三', u'二', u'前'), (u'前', u'２', u'３', u'４',
                                                      u'后'))

        #车马炮命名规则
        if man_species in [
                PieceT.ROOK, PieceT.CANNON, PieceT.KNIGHT, PieceT.PAWN
        ]:
            #红黑顺序相反，俩数组减少计算工作量
            count = 0
            pos_index = -1
    ...
    def to_iccs(self):
        return chr(ord('a') + self.p_from.x) + str(
            self.p_from.y) + chr(ord('a') + self.p_to.x) + str(self.p_to.y)

    @staticmethod
    def from_iccs(move_str):
        return (Pos(ord(move_str[0]) - ord('a'), int(move_str[1])), Pos(
