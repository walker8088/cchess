...
class TestPGN:
    ...
    def test_base(self):
        game = Game.read_from(Path("tests", "data", "test.pgn"))
    ...
    def test_qianhou(self):
        game = Game.read_from(Path("tests", "data", "中炮对列炮黑先平士角炮.pgn"))
    ...
    def test_long_move(self):
        ...
        game = Game.read_from(Path("tests", "data", "test2.pgn"))
        moves_all = game.dump_text_moves()
        move_line = move_t.split(",")
        assert len(moves_all) == 1
        moves = moves_all[0]
    ...
    def test_read_endgame(self):
        ...
        game = Game.read_from(Path("tests", "data", "endgame_test.pgn"))
