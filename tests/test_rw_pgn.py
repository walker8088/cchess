...
class TestPGN:
    ...
    def test_base(self):
        game = Book.read_from(Path("tests", "data", "test.pgn"))
        ...
        assert (
            move_line
            == "炮二平五,马２进３,马二进三,马８进９,车一进一,象３进５,兵五进一,炮８平６,兵五进一,卒５进１,马三进五,士４进５,车一平六,车１平４,车九进一,炮２进７,车六进八,将５平４,车九平六,将４平５,马五进七,炮６进３,马七进六,炮６平５,炮五进三"
        )

        assert len(moves) == 1
        assert game.verify_moves() is True

    def test_qianhou(self):
        game = Book.read_from(Path("tests", "data", "中炮对列炮黑先平士角炮.pgn"))
    ...
    def test_long_move(self):
        ...
        game = Book.read_from(Path("tests", "data", "test2.pgn"))
        moves_all = game.dump_text_moves()
        move_line = move_t.split(",")
        assert len(moves_all) == 1
        moves = moves_all[0]
        ...
        out_file = Path("tests", "data", "test2_out.pgn")

        game.save_to(out_file)

        game2 = Book.read_from(out_file)
    ...
    def test_read_endgame(self):
        ...
        game = Book.read_from(Path("tests", "data", "endgame_test.pgn"))
