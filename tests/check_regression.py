...
# 直接导入 benchmark 模块
import benchmark
from benchmark import BenchmarkResult, run_all_benchmarks

# 性能基线（允许 10% 的波动）
BASELINE = {
    "get_all_fench_positions": 0.030,  # ms
...
def main():
    ...
    if report["regressions"]:
        ...
        for reg in report["regressions"]:
            print(
                f"  {reg['name']}: {reg['baseline_ms']:.3f} ms -> {reg['current_ms']:.3f} ms ({reg['change_pct']:+.1f}%)"
            )
    ...
    if report["improvements"]:
        ...
        for imp in report["improvements"]:
            print(
                f"  {imp['name']}: {imp['baseline_ms']:.3f} ms -> {imp['current_ms']:.3f} ms ({imp['change_pct']:+.1f}%)"
            )
