...
```
[qualifier][piece_type][column][direction][distance][x?][+?][#?]
```
...
| 字段 | 说明 | 示例值 |
|------|------|--------|
| `qualifier` | 限定词（可选） | `f`（前）、`m`（中）、`b`（后） |
| `piece_type` | 棋子类型字母 | `R`（红车）、`c`（黑炮） |
| `column` | 列号（1-9） | `1`-`9` |
| `direction` | 方向符号 | `+`（进）、`-`（退）、`=`（平） |
| `distance` | 距离/目标列 | 步数或列号 |
| `x` | 吃子标记（可选） | 出现表示吃子 |
| `+` | 将军标记（可选） | 出现表示将军 |
| `#` | 将死标记（可选） | 出现表示将死 |
...
- 对于王、车、炮、兵（进退时）：表示步数（1-9）
- 对于马、士、象（进退时）：表示目标路数（1-9）
- 对于平移（`=`）：表示目标路数（1-9）
...
| `N8+2` | 马二进三 | 红方二路马进到三路 |
| `R1+1` | 车九进一 | 红方九路车前进1步 |
| `fR4+3` | 前车进三 | 红方四路前车前进3步 |
| `bR4+3` | 后车进三 | 红方四路后车前进3步 |
| `c2=4` | 炮２平５ | 黑方2路炮平到5路 |
| `R5+1#` | 车五进一（将死） | 红方五路车进1步，将死 |
| `C8+5x+` | 炮二进五（吃子将军） | 红方二路炮进5步，吃子且将军 |
